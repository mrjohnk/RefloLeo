...
void setup() {
  ...
  if(!sdcard.begin(SS, SPI_HALF_SPEED)){
    ...
    Serial.println("No SD Memory Card Detected ");
    ...
    for(fctr=0; fctr<25; fctr++){
      ...
      if(gotchar > 32 && gotchar < 123) {
        ...
        if(gotchar == 13) {
          ...
          gotchar = SDfile.read();
          if(gotchar != 10) 
            Serial.print("ERROR: did not get expected LF");  // in this case, will have to save character, etc
          }
        if(gotchar == 10 || gotchar == 13 || gotchar == 255) {
          buff[fctr]='\0';
          break;   
          }
