...
long CapacitiveSensor::capacitiveSensor(uint8_t samples)
{
	...
	//	if ( (millis() - lastCal > CS_AutocaL_Millis) && abs(total  - leastTotal) < (int)(.1 * (float)leastTotal) ) {
		if( calibCTR > samples * 2 ){
		  ...
		  if(abs(total  - leastTotal) > 10){
		    calibCTR++;
		    //Serial.print("calibCTR=");
		    //Serial.println(calibCTR);
		    }
		  else calibCTR=0;
		  }
...
void CapacitiveSensor::set_CS_Timeout_Millis(unsigned long timeout_millis){
	...
	CS_Timeout_Millis = (timeout_millis * (float)loopTimingFactor * (float)F_CPU) / 8000000;  // floats to deal with large numbers
}
...
int CapacitiveSensor::SenseOneCycle(void)
{
    ...
    *sOut |= sBit;		   // set send pin to OUTPUT     (JK)
	*sOut &= ~sBit;        // set Send Pin Register low
    //delayMicroseconds(50);				// let the send pin drain    (JK)
	...
	*rReg |= rBit;         // set pin to OUTPUT - pin is now LOW AND OUTPUT
	//delayMicroseconds(50); // let receive pin drain
	*rReg &= ~rBit;        // set pin to INPUT

    *sOut &= ~sBit;		   // set send pin to INPUT     (JK)
